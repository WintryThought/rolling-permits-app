...
export default function AuthScreen() {
  ...
  async function handleSignup(e) {
    ...
    // Create the vendor row tied to this new auth user. If email
    // confirmation is required, data.session will be null here —
    // the vendor row still gets created once they confirm and the
    // RLS policy (auth.uid() = auth_user_id) will match on first login.
    ...
    if (userId) {
      ...
      if (vendorError) {
        setError(
          `Account created, but saving your business details failed: ${vendorError.message}`
        );
      }
    }
