...
def fetch_due_coi_reminders(supabase: Client):
    ...
    missed cron run just means it fires a day late instead of being
    silently skipped.
    """
    cutoff = _date_n_days_from_today(COI_REMINDER_DAYS)
    resp = (
        supabase.table("coi_distributions")
        .select(
            "id, recipient_name, recipient_type, needed_by_date, additional_insured_required,"
            "permits(id, permit_type, vendors(business_name, email, phone))"
        )
        .lte("needed_by_date", cutoff)
        .is_("sent_date", "null")
...
def mark_coi_reminder_sent(supabase: Client, distribution_id: str):
    supabase.table("coi_distributions").update(
        {"reminder_sent_at": "now()"}
    ).eq("id", distribution_id).execute()


def send_coi_reminders(supabase: Client) -> int:
    sent_count = 0
    due = fetch_due_coi_reminders(supabase)

    for dist in due:
        permit = dist.get("permits")
        if not permit:
            continue
        vendor = permit.get("vendors") or {}
        if not vendor.get("email"):
            continue

        subject = f"Rolling Permits: COI needed for {dist['recipient_name']}"
        ai_note = (
            " They also require you to be listed as an additional insured — "
            "confirm that endorsement is in place before sending."
            if dist.get("additional_insured_required")
            else ""
        )
        body = (
            f"Hi {vendor['business_name']},\n\n"
            f"{dist['recipient_name']} needs your Certificate of Insurance "
            f"by {dist['needed_by_date']}, and it hasn't been marked as sent yet."
            f"{ai_note}\n\n"
            f"Log in to Rolling Permits to mark it sent once it's out the door.\n\n"
            f"— Rolling Permits"
        )
        send_email(vendor["email"], subject, body, body.replace("\n", "<br>"))
        mark_coi_reminder_sent(supabase, dist["id"])
        sent_count += 1
    ...
    for threshold in THRESHOLDS:
        ...
        for permit in permits:
            ...
            if vendor.get("email") and not already_sent(
                ...
                log_reminder(supabase, permit["id"], threshold, "email")
                sent_count += 1

            is_paid_tier = vendor.get("plan_tier") in ("single", "multi")

            if (
                vendor.get("phone")
                and is_paid_tier
                and twilio_configured()
                and not already_sent(supabase, permit["id"], threshold, "sms")
            ):
                # SMS is a paid-tier feature — free tier gets email only.
                # SMS gets a shorter version of the message
                sms_body = (
                    f"Rolling Permits: {PERMIT_TYPE_LABELS.get(permit['permit_type'], permit['permit_type'])} "
                    f"expires in {threshold} days ({permit['expiration_date']}). "
                    f"Renew now to stay compliant."
                )
                send_sms(vendor["phone"], sms_body)
                log_reminder(supabase, permit["id"], threshold, "sms")
                sent_count += 1

    coi_sent = send_coi_reminders(supabase)
    sent_count += coi_sent

    print(f"Reminder run complete. {sent_count} message(s) sent ({coi_sent} were COI recipient reminders).")
...
# ------------------------------------------------------------
# Backfill note:
# Because fetch_due_permits() matches on an EXACT day offset, a
# missed run (e.g. the cron didn't fire) means that threshold is
# silently skipped for any permit landing on that exact day. For
# a v1 with daily runs this is rare and low-stakes (the next
# threshold down will still catch it), but if you want zero-miss
# guarantees, change the .eq("expiration_date", ...) filter to
# .lte("expiration_date", ...) combined with the already_sent()
# idempotency check, which already protects against duplicates.
# ------------------------------------------------------------
